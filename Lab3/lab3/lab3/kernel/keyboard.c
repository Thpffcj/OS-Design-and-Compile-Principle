 ...
 PUBLIC void keyboard_handler(int irq)
{
	...
	if (kb_in.count < KB_IN_BYTES) {
		*(kb_in.p_head) = scan_code;
		kb_in.p_head++;
		if (kb_in.p_head == kb_in.buf + KB_IN_BYTES) {  //如果已经达到缓冲区末尾，则应将指针移到开头
...
//读取键盘缓存,函数需要了解自己被哪一个TTY调用,为函数传入TTY指针
PUBLIC u32 keyboard_read(TTY *p_tty)
{
	...
	if(kb_in.count > 0){  //如果键盘缓冲区有东西
		code_with_E0 = 0;  //对0XE0开头的扫描码做处理
		...
		if ((key != PAUSEBREAK) && (key != PRINTSCREEN)) {
			...
			make = (scan_code & FLAG_BREAK ? 0 : 1);  //FLAG_BREAK=0X80
			...
			case CAPS_LOCK: //大写键
				...
				break;
			case NUM_LOCK:  //锁定/解锁小键盘
				if (make) {
					num_lock    = !num_lock;
					set_leds();
				}
				break;
			case SCROLL_LOCK:  //滚动锁定键
				if (make) {
 ...
 *======================================================================*/
 ...
 //读缓冲区开始时关闭了中断，到结束时才打开，因为kb_in作为一个整体
